...
async def pro_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return
    ok = set_pro(uid, True)
    await update.message.reply_text("✅ PRO berildi." if ok else "Foydalanuvchi topilmadi.")
...
async def unpro_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("✅ PRO olib tashlandi." if ok else "Foydalanuvchi topilmadi.")
...
async def handle_pdf(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not HAS_FITZ:
        await update.message.reply_text(
            "ℹ️ PDF'ni rasmga ajratish uchun kutubxona yo'q. Termux'da: pip install pymupdf\n"
...
async def produce_pdf(context, chat_id, user, filename):
    ...
    if not allowed:
        await context.bot.send_message(
            ...
            "⭐ PRO bilan cheksiz foydalanish mumkin.",
        )
        return
...
async def handle_split(q, context):
    ...
    if not data:
        ...
        return
    if not HAS_FITZ:
        await q.edit_message_text("❌ Kutubxona yo'q: pip install pymupdf")
        return
    ...
    if not allowed:
        await q.edit_message_text(
            f"⛔ Bugungi limit tugadi ({FREE_DAILY_LIMIT}). ⭐ PRO bilan cheksiz."
        )
        return
